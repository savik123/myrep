...
import { OrbitControls } from "https://esm.sh/three@0.183.2/examples/jsm/controls/OrbitControls.js";
...
function getCurrentUserName() {
    return userBadgeName?.textContent?.trim() || "Гость";
}

function setUserBadgeName(name) {
    if (userBadgeName) {
        userBadgeName.textContent = name || "Гость";
    }
}
...
async function saveProfileName() {
    ...
    setUserBadgeName(data.name);
    closeProfileModal();
}

function openConfirmModal(message, okText = "Взять") {
...
if (confirmModal) {
    const backdrop = confirmModal.querySelector(".confirm-modal__backdrop");
    if (backdrop) {
...
if (profileModal) {
    const backdrop = profileModal.querySelector(".profile-modal__backdrop");
    if (backdrop) {
...
if (!getCurrentUserName() || getCurrentUserName() === "Гость") {
    openProfileModal();
}
...
machineCard.addEventListener("click", async (event) => {
    ...
    const confirmed = await openConfirmModal(`Взять «${itemName}» со склада?`, "Взять");
    if (!confirmed) return;
